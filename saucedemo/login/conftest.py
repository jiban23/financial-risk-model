"""Cross-browser driver setup for the login suite.

The login cases now run across every browser listed in ``--browsers``
(default: chrome, firefox, edge, opera). Unavailable browsers are SKIPPED
rather than failed. Options are declared in the repo-root conftest.py; the
driver factory lives in the repo-root browsers.py.
"""
import pytest
